...
export default class Bomb extends Container {
  ...
  // серверная строка приходит один раз — подтверждением локально
  ...
  destroy(options) {
    ...
    if (this._soundId) {
      // одноразовый сэмпл постановки живёт дольше самой бомбы
      // (её убирает детонация через weapon.time) — отпускаем, а не обрываем
