...
- **`panel`** — panel element ids and the mapping of server keys (`t`, `h`, `wa`, `w1`, `w2`) to elements; `health` (`blocks` — the number of health bar blocks, minimum 2; `animation.delay`/`animation.duration` — the round-start fill timings) parametrizes the health bar (see [client.md](client.md#mvc-components-srcclientcomponents)).
