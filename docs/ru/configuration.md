...
- **`panel`** — id элементов панели и сопоставление серверных ключей (`t`, `h`, `wa`, `w1`, `w2`) элементам; `healthAnimation` (`blocks`/`delay`/`duration`) параметризует заполнение полосы здоровья при старте раунда (см. [client.md](client.md#mvc-компоненты-srcclientcomponents)).
