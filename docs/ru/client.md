...
- `reset()` сбрасывает состояние **полностью** (включая `hasState`/`state`/`frozen`) — до прихода следующего player-блока рендерить нечего. При `CLEAR` и `MAP_DATA` вместе с ним обнуляется идентичность своего танка (`myGameId`/`myTankMeta` в `main.js`, хелпер `resetOwnTank()`), иначе предикт воскрешал бы танк на уже очищенном полотне (вместе с зациклённым звуком двигателя).
