...
- **Panel** — HUD: время раунда, здоровье, боезапас, активное оружие (по строкам `'ключ:значение'`). В начале раунда (`GAME_INFORM_DATA` с кодом `roundStart`) `main.js` проигрывает волновую CSS-анимацию на `#logo` (класс `logo-round-start`) и вызывает `PanelCtrl.playRoundStart()`, который заполняет полосу здоровья блок за блоком слева направо; обе анимации учитывают `prefers-reduced-motion`.
