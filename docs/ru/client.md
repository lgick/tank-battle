...
- держит идентичность (`gameId` из player-блока кадра, имя модели из формы авторизации) и дискретные поля снапшота, которых нет в предсказанном состоянии (`condition`/`armor`/`bullets`);
